...
<head>
<meta name="yandex-verification" content="3a052ddac5ec18f7" />
...
?>
<link rel="icon" href="<?php if( ! empty( $favicon ) ) { echo $favicon; } elseif( !empty( $niche_favicon ) ) { echo get_template_directory_uri() . $niche_favicon; } ?>">
...
<div id="page">
	...
	<header id="masthead" class="site-header" role="banner">
		...
		<div class="site-branding row">
			<div class="logo">
				...
				<?php else :
					...
					?>
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home" title="<?php echo get_bloginfo( 'name' ); ?>"><img src="<?php if( !empty( $image_logo_file ) ) { echo $image_logo_file; } elseif( !empty( $niche_image_logo_file ) ) { echo get_template_directory_uri() . $niche_image_logo_file; } ?>" alt="<?php echo get_bloginfo( 'name' ); ?>"></a>
				<?php endif; ?>
		...
		<nav id="site-navigation" class="main-navigation <?php if(xbox_get_field_value( 'wpst-options', 'enable-membership' ) == 'on') : ?>membership-enabled<?php endif; ?>" role="navigation">
			<div id="head-mobile"></div>
			<div class="button-nav"></div>
			<?php wp_nav_menu( array( 'theme_location' => 'wpst-main-menu', 'menu_class' => 'row', 'container' => false ) ); ?>
		</nav><!-- #site-navigation -->
	...
	<?php if ( function_exists('wpst_breadcrumbs') && xbox_get_field_value( 'wpst-options', 'enable-breadcrumbs' ) == 'on' ) {
		wpst_breadcrumbs();
	} ?>

	<?php if( xbox_get_field_value( 'wpst-options', 'show-videos-carousel' ) == 'on' && is_home() ) { get_template_part( 'template-parts/content', 'featured-carousel' ); } ?>
