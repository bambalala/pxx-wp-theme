...
<?php if( xbox_get_field_value( 'wpst-options', 'show-sidebar' ) == 'on') {
	...
	<div id="primary" class="content-area <?php echo $sidebar_pos; ?>">
		...
		<?php if ( have_posts() ) { ?>
			<header class="page-header">
				<?php
					...
					if( xbox_get_field_value( 'wpst-options', 'tag-desc-position' ) == 'top') { the_archive_description( '<div class="archive-description">', '</div>' ); }
				?>
		...
		if( xbox_get_field_value( 'wpst-options', 'tag-desc-position' ) == 'bottom' ) : ?><div class="clear"></div><?php the_archive_description( '<div class="archive-description">', '</div>' ); ?><?php endif; ?>
		</main><!-- #main -->
	</div><!-- #primary -->
<?php
get_sidebar();
